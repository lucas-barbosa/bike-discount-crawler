...
class BarrabesDeleteProductsByCategoryJob {
  public static function register() {
    add_action('lb_barrabes_delete_products_by_category_async', [__CLASS__, 'dispatch'], 10, 2);
    add_action('lb_barrabes_delete_products_by_category_job', [__CLASS__, 'handle'], 10, 4);
  }
  ...
  public static function dispatch($category, $category_path = []) {
    ...
    $existing = as_next_scheduled_action('lb_barrabes_delete_products_by_category_job', $args, 'lb-crawler');
    if (!$existing) {
      as_schedule_single_action(time() + 5, 'lb_barrabes_delete_products_by_category_job', $args, 'lb-crawler');
    }
  }
  ...
  public static function handle($category, $category_path = [], $batch_size = 20, $term_id = null) {
    ...
    if (count($product_ids) === $batch_size) {
      as_schedule_single_action(time() + 5, 'lb_barrabes_delete_products_by_category_job', [$category, $category_path, $batch_size, $term_id], 'lb-crawler');
    } else {
  ...
  private static function findTermIdFromHierarchy($category_path) {
    ...
    foreach ($category_path as $cat_name) {
      ...
      $term_id = BarrabesMapper::getTermId($cache_key);
