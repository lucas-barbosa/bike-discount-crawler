...
class TradeInnDeleteProductsByCategoryJob {
  public static function register() {
    add_action('lb_tradeinn_delete_products_by_category_async', [__CLASS__, 'dispatch'], 10, 2);
    add_action('lb_tradeinn_delete_products_by_category_job', [__CLASS__, 'handle'], 10, 4);
  }
  ...
  public static function dispatch($category, $category_path = []) {
    ...
    $existing = as_next_scheduled_action('lb_tradeinn_delete_products_by_category_job', $args, 'lb-crawler');
    if (!$existing) {
      as_schedule_single_action(time() + 5, 'lb_tradeinn_delete_products_by_category_job', $args, 'lb-crawler');
    }
  }
  ...
  public static function handle($category, $category_path = [], $batch_size = 20, $term_id = null) {
    ...
    // Se term_id não foi passado, busca
    if (!$term_id) {
    ...
    // Buscar lote de produtos vinculados a esse category_id
    ...
    if (count($product_ids) === $batch_size) {
      as_schedule_single_action(time() + 5, 'lb_tradeinn_delete_products_by_category_job', [$category, $category_path, $batch_size, $term_id], 'lb-crawler');
    } else {
  ...
  /**
   * Encontra o term_id final navegando pela hierarquia de categorias
   * Usa o TradeInnMapper e SettingsData específicos
   */
  private static function findTermIdFromHierarchy($category_path) {
    ...
    foreach ($category_path as $cat_name) {
      ...
      $term_id = TradeInnMapper::getTermId($cache_key);
      if ($term_id) {
